...
def _req(tmp_path: Path, finding: dict, fixed_content: str = "password = 'fixed'\n") -> creator.PRRequest:
...
def test_empty_fix_content_skips(tmp_path: Path, finding: dict):
    req = _req(tmp_path, finding, fixed_content="   \n")
    result = creator.create_pr(req, validate=lambda _: True, client=MagicMock())
    assert result.created is False
    assert result.skipped_reason == "empty fix content"


def test_missing_file_path_skips(tmp_path: Path):
    finding_no_file = {"scanner": "bandit", "rule_id": "B105", "severity": "high"}
    req = _req(tmp_path, finding_no_file)
    result = creator.create_pr(req, validate=lambda _: True, client=MagicMock())
    assert result.created is False
    assert result.skipped_reason == "finding has no file path"


def test_failing_validation_aborts(tmp_path: Path, finding: dict, monkeypatch):
    calls = {"write": 0, "create_branch": 0, "commit": 0, "push": 0, "pr": 0, "reset": 0}
    ...
    def fake_write(path, target_relpath, content):
        calls["write"] += 1
    ...
    monkeypatch.setattr(creator.committer, "write_full_file", fake_write)
...
def test_working_tree_isolated_across_sequential_findings_real_git(tmp_path: Path, monkeypatch):
    ...
    interaction, which the mocked tests above can't. Fix content is a full
    file replacement (not a diff), matching what fix_single_file.j2 actually
    asks the LLM for and what committer.write_full_file actually does with
    it — a previous version of this test used diff-shaped strings that
    masked the CRITICAL bug where write used to be `git apply`."""
    ...
    fixed1 = "a = 2  # finding1 leftover\n"
    fixed2 = "b = 2  # finding2 fix\n"

    req1 = creator.PRRequest(
        finding=finding1, fixed_content=fixed1, confidence=0.8, model_used="deepseek",
    ...
    # finding #1's fix is applied-but-uncommitted on disk at this point —
    ...
    try:
        req2 = creator.PRRequest(
            finding=finding2, fixed_content=fixed2, confidence=0.9, model_used="deepseek",
            repo_path=repo, repo_full_name="owner/repo", base_branch="main",
