...
_FAKE_PR_BODY = (
    ...
    "**Scanner:** bandit\n**Rule:** B105\n**Severity:** high\n**CWE:** CWE-259\n\n"
    "### What was found\nA hardcoded credential was detected.\n\n"
    "### What was changed\nThe credential now comes from an environment variable.\n\n"
    "### Confidence\n0.82/1.0 (deepseek-coder:6.7b)\n\n"
    "---\n*Generated by LLM-Assisted CI/CD Security Agent*\n"
)


class _FakeHaiku:
    """Haiku is called for three different templates (classify_error.j2,
    confidence_eval.j2, pr_body.j2) — route by a substring unique to each so
    a high confidence_eval score can stop escalation at attempt 1 (matching
    what a real independent evaluator would do), and so pr_body.j2's
    unfilled-placeholder prompt text is never mistaken for the actual PR
    body (a prior bug returned the rendered *prompt* directly as the body
    instead of ever calling an LLM with it)."""

    def __init__(self, classify_text: str, eval_text: str, pr_body_text: str = _FAKE_PR_BODY):
        self._classify_text = classify_text
        self._eval_text = eval_text
        self._pr_body_text = pr_body_text
    ...
    def complete(self, prompt, max_tokens, temperature=0.2):
        ...
        elif "pull request description" in prompt:
            text = self._pr_body_text
        else:
